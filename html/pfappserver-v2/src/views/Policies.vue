<script setup>
// Policies / Connection Profiles — ported from the design's policies.jsx.
// Left rail = profile list, right = flow diagram (Match → Authenticate →
// Authorize) plus the full edit form. Mock-only for now; PF's endpoint is
// /api/v1/config/connection_profiles, wired in a follow-up.
  ...
  <div class="page" style="max-width: 1480px">
    <div class="page-head">
      <div>
        ...
        <div class="page-sub">Connection profiles · authentication sources · roles</div>
      </div>
      <div class="page-tools">
        <button class="btn"><Icon name="download" :size="13" /> Export</button>
        <button class="btn primary"><Icon name="plus" :size="13" /> New profile</button>
      </div>
    </div>

    <div class="tabs">
      <div class="t active">Connection profiles <span class="chip" style="margin-left:6px">{{ profiles.length }}</span></div>
      <div class="t">Authentication sources <span class="chip" style="margin-left:6px">6</span></div>
      <div class="t">Roles <span class="chip" style="margin-left:6px">{{ NAC_DATA.roles.length }}</span></div>
      <div class="t">Portal modules</div>
      <div class="t">Provisioners</div>
    </div>
